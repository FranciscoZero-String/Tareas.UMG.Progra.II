...
import DATOS.MdEmpleado;
import DATOS.NominaEmpleados;

public class Principal implements Initializable {
    private final String errorMessage = "ERROR.";
    private final String emptyFieldMessage = "Seleccion no valida";
    ...
    void eventGuardar(ActionEvent event) {
        try {
            if (checkTextFields()) {
                ...
                new Alert(Alert.AlertType.INFORMATION, "LLENE LOS CAMPOS").showAndWait();
            }
    ...
    void eventModificar(ActionEvent event) {
        try {
            if (!txtCodigo.getText().isBlank()) {
                ...
                Double.valueOf(txtFebrero.getText()), Double.valueOf(txtMarzo.getText()));
                new Alert(Alert.AlertType.INFORMATION, "REGISTRO MODIFICADO").showAndWait();
            } else {
                new Alert(Alert.AlertType.INFORMATION, "BUSCAR REGISTRO.").showAndWait();
            }
        } catch (NumberFormatException ex) {
            new Alert(Alert.AlertType.ERROR, errorMessage).showAndWait();
        } finally {
        ...
        try {
            if (!txtBuscarCantidad.getText().isBlank()) {
                ...
                if (cantidad != null) {
                    ...
                    resultado = " NO SE HALLARON RESULTADOS";
                }
